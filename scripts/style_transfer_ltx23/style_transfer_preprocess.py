...
def _copy_reference_image_to_intermediates(reference_image: Path, output_path: Path) -> Path:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    if reference_image.resolve() == output_path.resolve():
        return output_path
    shutil.copy2(reference_image, output_path)
    print(f"Copied reference image to {output_path}")
    return output_path
...
def resolve_reference_image(args: argparse.Namespace, source_first_frame: Path, work_dir: Path, frame_idx: int = 0) -> Path:
    if args.translated_first_frame:
        ...
        reference_image = Path(args.translated_first_frame).expanduser().resolve()
        if not reference_image.is_file():
            raise FileNotFoundError(f"translated-first-frame does not exist: {reference_image}")
        translated = _translated_reference_output_path(args, work_dir, frame_idx)
        return _copy_reference_image_to_intermediates(reference_image, translated)
...
def add_preprocess_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--translated-first-frame", "--reference-image-path", dest="translated_first_frame", default=None)
